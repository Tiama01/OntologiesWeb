...
import EthniesParRegionPage from './components/pages/EthniesParRegionPage';
import ToutesEthniesPage from './components/pages/ToutesEthniesPage';
...
const fetchAllData = async () => {
  try {
    const [ethniesRes, languesRes, regionsRes, patronymesRes, dialectesRes] = await Promise.all([
      fetch('http://localhost:8000/api/neo4j/ethnies', { signal: AbortSignal.timeout(10000) }),
      fetch('http://localhost:8000/api/neo4j/langues', { signal: AbortSignal.timeout(10000) }),
      fetch('http://localhost:8000/api/neo4j/regions', { signal: AbortSignal.timeout(10000) }),
      fetch('http://localhost:8000/api/neo4j/patronymes', { signal: AbortSignal.timeout(10000) }),
      fetch('http://localhost:8000/api/neo4j/dialectes', { signal: AbortSignal.timeout(10000) })
    ]);

    const [ethniesData, languesData, regionsData, patronymesData, dialectesData] = await Promise.all([
      ethniesRes.ok ? ethniesRes.json() : [],
      languesRes.ok ? languesRes.json() : [],
      regionsRes.ok ? regionsRes.json() : [],
      patronymesRes.ok ? patronymesRes.json() : [],
      dialectesRes.ok ? dialectesRes.json() : []
    ]);
    ...
    return {
      ...
      langues: languesData || [],
      regions: regionsData || [],
      patronymes: patronymesData || [],
      dialectes: dialectesData || []
    };
  } catch (error) {
    console.error('Erreur récupération données:', error);
    // Retour de tableaux vides en cas d'erreur au lieu de données statiques
    return {
      ethnies: [],
      langues: [],
      regions: [],
...
const DashboardEnhanced = () => {
  ...
  const stats = {
    ...
    total_dialectes: allData.dialectes?.length || 0,
    total_locuteurs: allData.langues?.reduce((sum, l) => sum + (l.nombre_locuteurs || 0), 0) || 0,
    ethnies_majoritaires: allData.ethnies?.filter((e) => e.statut_national === 'MAJORITAIRE').length || 0,
    langues_principales: allData.langues?.filter((l) => l.statut === 'principale').length || 0,
    regions_avec_gps: allData.regions?.filter((r) => r.latitude && r.longitude).length || 0,
    patronymes_documentes: allData.patronymes?.filter((p) => p.signification).length || 0,
    dialectes_documentes: allData.dialectes?.filter((d) => d.description).length || 0
  };
    ...
    <div className="space-y-6 p-6">
      ...
      >
        <div>
          <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
            Ontologie des langues et ethnies du BF
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <StatCardEnhanced
          ...
          change={`${Math.round((stats.patronymes_documentes / (stats.total_patronymes || 1)) * 100)}% documentés`}
          changeType="positive"
        ...
        <StatCardEnhanced
          title="Dialectes régionaux"
          value={stats.total_dialectes.toString()}
          subtitle={`${stats.dialectes_documentes} avec description`}
          change={`${Math.round((stats.dialectes_documentes / (stats.total_dialectes || 1)) * 100)}% documentés`}
          changeType="positive"
          ...
          onClick={() => handleStatCardClick('dialectes')}
          isExpandable={true}
        />
      </div>
      ...
      {(showAllCharts || expandedWidget) && (
        <div className="space-y-6">
          ...
          {(showAllCharts || expandedWidget === 'dialectes') && (
            <ChartWidget
              title="🌍 Analyse des Dialectes"
              type="dialectes"
              data={allData.dialectes || []}
              defaultExpanded={expandedWidget === 'dialectes'}
            />
          )}
        </div>
      )}
...
function App() {
  ...
  const renderCurrentPage = () => {
    switch (currentPage) {
      ...
      case 'carte':
        return <CartePage />;
      case 'ethnies-par-region':
        return <EthniesParRegionPage />;
      case 'toutes-ethnies':
        return <ToutesEthniesPage />;
      default:
        return <DashboardEnhanced />;
    }
  };
